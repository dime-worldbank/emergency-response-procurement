...
##________________Tender data_____________________________ 
...
class(tender_contract$sub)


#Generate the dummy for the treatment
n_tender <- tender_contract %>%
...
n_tenders_y_covid <- data_covid[year > 0,
                                  list(
                                  n_c                     = .N                                    ,
                                  decision                = mean(decision_time                 , na.rm = TRUE),
                                  submission              = mean(submission_time               , na.rm = TRUE),
                                  processing              = mean(processing_time               , na.rm = TRUE)),
...
for(dep_var in c( 
  ...
  title = case_when(
    dep_var == "decision"~ "Average number of days between end of bidding and awarding",                
    dep_var == "submission" ~ "Average number of days between start and end of bidding",              
    dep_var == "processing" ~ "Average number of days between start of bidding and awarding contract"
...
model_ols(data = n_tenders_y_covid, 
          dep_var = dep_var,
          title = title, 
...
##__________________Contract method data__________________________________________  

#Adjust data
SHARES 

method_share <- as.data.table(method_share)

method_share$year_month <- as.Date(paste0(method_share$year, "-", method_share$month, "-01"))

#Adjust the data
method_share<- method_share%>%
  mutate(method_fa = as.factor(method),
         ID_CONTRACT_fa = as.factor(ID_CONTRACT)) 

method_share <- method_share %>%
  group_by(year_month, Group)%>%
  mutate(total_value = sum(AMT_CONTRACT_VALUE))%>%
  mutate(frequency_con = length(unique(ID_CONTRACT_fa)))

method_share <- method_share %>%
  group_by(year_month, Group, method)%>%
  mutate(sum_frequency = sum(unique(AMT_CONTRACT_VALUE)))


method_share <- method_share%>%
  group_by(year_month, Group, method_fa)%>%
  mutate(share_freq = (sum_frequency/frequency_con)*100)%>%  
  mutate(share_value = (AMT_CONTRACT_VALUE_USD/total_value)*100)


#Generate the same dummy Ruggero's has for treatment
method_share <- method_share %>%
  mutate(COVID_LABEL = case_when(covid_tender == 1 ~ 1,
                                 covid_tender == 0 ~ 0)) 
...
data_covid <- method_share %>% 
  filter(YEAR > 2014)%>%
  mutate(treat = as.character(COVID_LABEL))%>%
  mutate(treat = COVID_LABEL == 1)


method_share_y_covid <- data_covid[year > 0,
                                  list(
                                  n_c                      = .N                            ,
                                  contract_value           = mean(AMT_CONTRACT_VALUE_USD   , na.rm = TRUE),
                                  share_freq               = mean(share_freq               , na.rm = TRUE),
                                  share_value              = mean(share_value              , na.rm = TRUE)),
                                  by = list(DT_M, DT_S, ID_BUYER, treat)] %>%
                                  mutate(Treated = treat == TRUE & DT_S %in% seq(8, 14)) ##ISSUE DON'T KNOW, ASK RUGGERO 
...
for(dep_var in c( 
  ...
  title = case_when(
    dep_var == "contract_value" ~ "Average contract value in USD",                                                   
    dep_var == "share_freq" ~ "Share of close purchases", ##ASK RUGGERO WHY HE REPORTS ONLY THE  DIRECT PURCHASES, HOW IT IS CODED. this will need a note to clarify what is close and what is open
    dep_var == "share_value" ~ "Share of value contract"
  )
  ...
  model_ols(data = method_share_y_covid, 
            dep_var = dep_var,
            title = title, 
            filename = paste0(dep_var, "_treat_"))
