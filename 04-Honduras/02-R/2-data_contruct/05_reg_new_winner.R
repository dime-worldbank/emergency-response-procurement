...
plot_bar_errors <- function(clfe, title) {
  ...
  plot <- ggplot()  +
    ...
    labs(caption = "The fixed effects included are monthly.") +
...
model_ols <- function(data, dep_var, filename, title) {
  ...
  clfe <- list(feols(as.formula(paste0(dep_var, "~ Treated | DT_M")), 
                     data = data, vcov = "HC1"),
               feols(as.formula(paste0(dep_var, "~ Treated | DT_M")), 
                     data = data, vcov = "HC1"))
...
new_winner  <- as.data.frame(fread(file.path(cleaned, "New_winner.csv"))) ##this is at contract level, so supplier and at month time. 
...
data_covid <- new_winner_sh %>% 
  filter(YEAR > 2014)%>%
  mutate(treat = COVID_LABEL  == 1) 

new_winner_sh_covid <- data_covid[year > 0,
                                list(
                                  n_c                     = .N                                    ,
                                  sh_new_winner        = mean(share_new_winner                 , na.rm = TRUE)),
                                by = list(DT_M, DT_S, treat)] %>%
...
for(dep_var in c( 
  ...
  title = case_when(
    dep_var == "sh_new_winner"~ "Share of new winners"
  ...
  model_ols(data = new_winner_sh_covid, 
            dep_var = dep_var,
            title = title, 
