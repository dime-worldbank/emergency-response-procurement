...
```{r outlier, echo = TRUE}
      ...
      # filter out contrats that have top 1% contract values
      ...
      outlier_detail <- tender_contract%>%filter(outliers == "FLAG")
        # end up singling out 138 observations

      fwrite(outlier_detail, file = paste0(cleaned, "/Outlier.CSV"))
      
      # generate a list of outliers 
      outlier <- outlier_detail %>%
        select(ID, ID_CONTRACT)
      
      # remove utliers from all intermediate files 
...
```{r 2.1 identify COVID, echo=TRUE}
...
```{r 2.2 identify COVID, echo=TRUE}
...
```{r 2.3 identify COVID, echo=TRUE}
...
```{r 2.4 identify COVID, echo=TRUE}
...
```{r 3.1 flag COVID item, echo=TRUE}
...
```{r 3.2 construct medical related dummies, echo=TRUE}
...
```{r 3.3 assign to comparison group, echo=TRUE}
...
```{r 4.1 construct new winners, echo=TRUE}
...
```{r 4.2 construct new winners, echo=TRUE}
...
```{r 5.1 time, echo=TRUE, warning=FALSE} 
...
```{r 5.2 submission time, echo=TRUE, warning=FALSE}
...
```{r 5.3 processing time, echo=TRUE, warning=FALSE}
...
```{r 5.4 decision time, echo=TRUE, warning=FALSE}
...
```{r 6.1 market concentration share}
...
```{r 6.2 total number of tenders per month}
