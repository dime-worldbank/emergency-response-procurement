...
# LOAD DATA -------------------------------------------------------------------
    ...
    data_releases      <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_releases.csv"             )))
    # load the parties data
    data_parties       <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_parties.csv"               ))) 
    # load the department data
    data_par_member    <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_par_memberof.csv"          ))) 
    # load the items data
    data_ten_items     <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_ten_items.csv"             ))) 
    # load the fees data
    data_ten_fees      <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_ten_participationfees.csv" ))) 
    # load the doc award data
    data_awa_doc       <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_awa_documents.csv"         ))) 
    # load the contract-guarantees data
    data_garantees_con <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_con_guarantees.csv"        ))) 
    # load the documents contract data
    data_document_con  <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_con_documents.csv"         ))) 
    ...
    data_contracts     <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_contracts.csv"             ))) 
    # load the supplier contract data
    data_suppliers_con <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_con_suppliers.csv"         ))) 
    # load the tender document data
    data_document_ten  <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_ten_documents.csv"         ))) 
    # load the tender document data
    data_awa_items     <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_awa_items.csv"             ))) 
    # load the tender document data
    data_sources       <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_sources.csv"               ))) 
    # load the tender document data
    data_awa_suppliers <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_awa_suppliers.csv"         ))) 
    # load the tender document data
    data_awards        <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_awards.csv"                ), encoding = "Latin-1", colClasses = "character")) 
    # load the tender document data
    data_ten_tenderers <- as.data.frame(fread(file.path(raw_oncae_interm,"DCC_ten_tenderers.csv"         ))) 
    # load data un
...
# PREPARE DATASETS -----------------------------------------------------------
    ...
    contract_tender <- left_join(data_contracts_final, data_tenders_final,
                                 by = "ID")
