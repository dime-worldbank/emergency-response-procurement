...
* 1: Competition Covid tenders - Restricting to autions
{
	...
	* Graph 01: 1/N_bidders
	format %10.2fc avg_S2_participants
		*  ${covid_shadow} 
	...
	* local y_dep avg_n_participants
	local y_dep avg_S2_participants

	tw 		(scatter `y_dep'  year_semester if ${group_covid} == 3 , connect(l) sort mcolor("98 190 121")   lcolor("98 190 121")  lp(solid)  ) ///
		|| 	(scatter `y_dep'  year_semester if ${group_covid} == 2 , connect(l) sort mcolor("104 172 32")   lcolor("104 172 32")  lp(dot)  msize(small) ) ///
		||	(scatter `y_dep'  year_semester if ${group_covid} == 1 , connect(l) sort mcolor("180 182 26")   lcolor("180 182 26")  lp(dot)  msize(small) ) ///
		|| 	(scatter `y_dep'  year_semester if ${group_covid} == 0 , connect(l) sort mcolor("233 149 144")  lcolor("233 149 144") lp(dash)  ) ///
		, ${opt_semester}  legend(order( 1 "High Covid" 2 "Medium Covid" 3 "low Covid" 4 "No Covid")  col(4))   		///
		note("") 
