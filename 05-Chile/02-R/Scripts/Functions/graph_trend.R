graph_trend <- function(
  ...
  if (yearly == FALSE) {
    ...
    plot <- ggplot() +
      ...
      geom_point(data = data %>% filter(!!treatment == 1 & DT_Q != 9), aes(x = DT_Q, y = !!variable), shape = 16, size = 3, color = "#FF0100") +
      geom_point(data = data %>% filter(!!treatment == 0 & DT_Q != 9), aes(x = DT_Q, y = !!variable), shape = 18, size = 4, color = "#18466E") +
      geom_line(data = data %>% filter(!!treatment == 1), aes(x = DT_Q, y = !!variable), size = 0.7, color = "#FF0100") +
      geom_line(data = data %>% filter(!!treatment == 0), aes(x = DT_Q, y = !!variable), size = 0.7, color = "#18466E", linetype = 2)  +
      labs(title    = title,
           subtitle = subtitle,
           caption  = caption,
           x = NULL,
    ...
    plot <- ggplot() +
      ...
      geom_line(data = data %>% filter(!!treatment == 1), aes(x = as.numeric(DT_Y), y = !!variable), linewidth = 0.7, color = "#FF0100") +
      geom_line(data = data %>% filter(!!treatment == 0), aes(x = as.numeric(DT_Y), y = !!variable), linewidth = 0.7, color = "#18466E", linetype = 2) +
      labs(title    = title,
           subtitle = subtitle,
           caption  = caption,
           x = NULL,
      ...
      annotate("segment", color = "#FF0100", x = 2020.5, xend = 2021, y = legend_upper + legend_upper * 0.03, yend = legend_upper + legend_upper * 0.03, size = 1) +
      annotate("segment", linetype = 2, color = "#18466E", x = 2020.5, xend = 2021, y = legend_upper - legend_upper * 0.03, yend = legend_upper - legend_upper * 0.03, size = 1) +
      geom_point(aes(x = 2020.75, y = legend_upper + legend_upper * 0.03), shape = 16, size = 3, color = "#FF0100") +
      geom_point(aes(x = 2020.75, y = legend_upper - legend_upper * 0.03), shape = 18, size = 4, color = "#18466E") +
      geom_text( family = "Roboto", fontface = "bold", x = 2021.2, y = legend_upper + legend_upper * 0.03, aes(label = label_treatment_legend)) +
      geom_text( family = "Roboto", fontface = "bold", x = 2021.2, y = legend_upper - legend_upper * 0.03, aes(label = label_control_legend))
    ...
    if (percentage == TRUE) {
      ...
      plot <- plot +
        geom_text(
          data = data.frame(x = 2021.5, y = seq(limit_lower, limit_upper, by = interval_limits_y)),
          aes(x, y, label = paste0(y, "%")),
          hjust = 1, # Align to the right
          vjust = - 0.5, # Align to the bottom
          size = 5
