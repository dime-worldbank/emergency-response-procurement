graph_trend <- function(
  ...
  if (yearly == FALSE) {
    ...
    if (percentage == TRUE) {
      ...
      plot <- plot +
        geom_text(
          data = data.frame(x = 15, y = seq(limit_lower, limit_upper, by = interval_limits_y)),
    ...
    plot <- ggplot() +
      ...
      annotate("segment", color = "#FF0100", x = 2021.5, xend = 2022, y = legend_upper + legend_upper * 0.03, yend = legend_upper + legend_upper * 0.03, size = 1) +
      annotate("segment", linetype = 2, color = "#18466E", x = 2021.5, xend = 2022, y = legend_upper - legend_upper * 0.03, yend = legend_upper - legend_upper * 0.03, size = 1) +
    ...
    if (percentage == TRUE) {
      ...
      plot <- plot +
        geom_text(
          data = data.frame(x = 2022.5, y = seq(limit_lower, limit_upper, by = interval_limits_y)),
          aes(x, y, label = paste0(y, "%")),
          hjust = 1, # Align to the right
          vjust = - 0.5, # Align to the bottom
          size = 5
