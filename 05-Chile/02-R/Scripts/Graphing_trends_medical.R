...
ggsave(
  filename = file.path(dropbox_dir, "Outputs/trend_n_contracts_health.png"),
  plot = plot                                                 ,
  width    = 11                                            ,
  height  = 7                                             ,
  dpi      = 250                                              ,
...
plot <- graph_trend(
  ...
  title = "Contracted (órdenes de compra) amount in millions (usd) per semester",
  caption = "Source: Chile Compra",
  limit_lower = 0,
  limit_upper = 4500,
...
ggsave(
  filename = file.path(dropbox_dir, "Outputs/trend_volume_contracts_health.png"),
  plot = plot                                                 ,
  width    = 11                                            ,
  height  = 7                                             ,
  dpi      = 250                                              ,
...
n_bidders_s_medicine <- data_offer_sub[DT_TENDER_YEAR > 2015, 
                                             ...
                                             by = list(DT_S, ID_ITEM, CAT_MEDICAL)]
...
last_bid_medicine <- data_offer_sub %>% 
  ...
  filter(DT_Y > 2014) %>% 
  distinct(ID_RUT_FIRM, ID_ITEM, DT_OFFER_SEND, DT_Y, DT_M, DT_S, CAT_MEDICAL) %>% 
  mutate(
    MONTHS = DT_Y * 12 + DT_M
  ) %>% 
  distinct(ID_RUT_FIRM, ID_ITEM, MONTHS, DT_M, DT_Y, DT_S, CAT_MEDICAL) %>% 
  arrange(
    MONTHS, ID_RUT_FIRM
  ) %>% 
  na.omit() %>% 
...
last_bid_s_medicine <- last_bid_medicine %>% 
  ...
  group_by(DT_S, ID_ITEM, CAT_MEDICAL) %>% 
...
plot <- graph_trend(
  ...
  treatment = CAT_MEDICAL, 
  title = "Volume contracted (órdenes de compra) contracted through direct",
  caption = "Source: Chile Compra",
  label_treatment_legend = "Medical",
  label_control_legend = "Non Medical",
  limit_lower = 0,
...
last_bid_medicine <- data_offer_sub %>% 
  ...
  filter(DT_Y > 2014) %>% 
  distinct(ID_RUT_FIRM, ID_ITEM, ID_RUT_BUYER, DT_OFFER_SEND, DT_Y, DT_M, DT_S, CAT_MEDICAL) %>% 
  mutate(
    MONTHS = DT_Y * 12 + DT_M
  ) %>% 
  distinct(ID_RUT_FIRM, ID_ITEM, ID_RUT_BUYER, MONTHS, DT_M, DT_Y, DT_S, CAT_MEDICAL) %>% 
  arrange(
...
last_win_medicine <- data_offer_sub %>% 
  ...
  distinct(ID_RUT_FIRM, ID_RUT_BUYER, ID_ITEM, MONTHS, DT_M, DT_Y, DT_S, CAT_MEDICAL) %>% 
  arrange(
    MONTHS, ID_RUT_FIRM, ID_RUT_BUYER
  ) %>% 
  na.omit() %>% 
